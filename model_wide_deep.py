...
class DnnModel():
  ...
  def model(self):
    ...
    y_hat = self.get_dnn_output(data_embed_concat, weight_list, bias_list)
    ...
    regularizer = tf.nn.l2_loss(wide_data_embed_concat)
    loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=y_hat,\
                          labels=y_target)) + 0.001 * regularizer
    return loss, y_hat, x_input, y_target, self.keep_prob, loss


  def __init__(self,n_ft,k,layer_dim,n_one_hot_slot,n_mul_hot_slot,max_len_per_slot,num_csv_col, \
                     wide_one_hot_index,wide_mul_hot_index,mem_size,mem_fea_index):
    # embedding sharing
    self.n_ft = n_ft
    self.k = k
    ## dataset
